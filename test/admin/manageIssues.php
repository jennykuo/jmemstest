...
*/
 ...
 function assert_handler($file, $line, $code)
  ...
  if (isset($HTTP_POST_VARS['action']) && ($HTTP_POST_VARS['action'] == 'processDeleteIssues')) {
		$issueIDs = split('@',$HTTP_POST_VARS["issuesToDelete"]);
		foreach($issueIDs as $issueID)
			tep_db_query("DELETE FROM issues WHERE ID='" . $issueID . "'");						
  }
  
  if (isset($HTTP_POST_VARS['action']) && ($HTTP_POST_VARS['action'] == 'processEditIssue')) {
		$issue = split('@',$HTTP_POST_VARS["issueToEdit"]);
		$issueID = $issue[0];	  
		$issueVolume = $issue[1];
		$issueNumber = $issue[2]; 
		$issueSeason = $issue[3]; 		
  ...
  if (isset($HTTP_POST_VARS['action']) && ($HTTP_POST_VARS['action'] == 'processAddIssue')) {
		$issue = split('@',$HTTP_POST_VARS["issueToAdd"]);  
		$issueVolume = $issue[0];
		$issueNumber = $issue[1]; 
		$issueSeason = $issue[2]; 		
		$issueYear = $issue[3]; 
