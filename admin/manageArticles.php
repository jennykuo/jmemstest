...
*/
 ...
 function assert_handler($file, $line, $code)
  ...
  assert($_SESSION["sessionUser"] == SITE_MANAGEMENT_USERNAME); 
	...
	$issue_query = tep_db_query("SELECT * FROM issues WHERE ID=$issueGet");
	$issueForArticles = tep_db_fetch_array($issue_query);  
  
	$patterns = array ("/~/","/'/");
	$replacement = array ("\"","'");  
  
  if (isset($_POST['action']) && ($_POST['action'] == 'processDeleteArticles')) {
		$ArticleIDs = split('@',$_POST["ArticlesToDelete"]);
		foreach($ArticleIDs as $ArticleID) {
			tep_db_query("DELETE FROM articles WHERE ID=" . (int) mysql_real_escape_string($ArticleID) . "");						
                }
  }
  
  if (isset($_POST['action']) && ($_POST['action'] == 'processEditArticle')) {
		$Article = split('@',$_POST["ArticleToEdit"]);
		$ArticleID = mysql_real_escape_string($Article[0]);	  
		$ArticleStartPage = mysql_real_escape_string($Article[1]);
		$ArticleEndPage = mysql_real_escape_string($Article[2]); 
		$ArticleTitle = mysql_real_escape_string(preg_replace($patterns,$replacement,$Article[3])); 		
		$ArticleAuthor = mysql_real_escape_string(preg_replace($patterns,$replacement,$Article[4])); 
		$ArticleAbstract = mysql_real_escape_string(preg_replace($patterns,$replacement,$Article[5])); 
  ...
  if (isset($_POST['action']) && ($_POST['action'] == 'processAddArticle')) {
		$Article = split('@',$_POST["ArticleToAdd"]);  
		$IssueID = mysql_real_escape_string($Article[0]);
		$ArticleStartPage = mysql_real_escape_string($Article[1]);
		$ArticleEndPage = mysql_real_escape_string($Article[2]); 
		$ArticleTitle = mysql_real_escape_string(preg_replace($patterns,$replacement,$Article[3])); 		
		$ArticleAuthor = mysql_real_escape_string(preg_replace($patterns,$replacement,$Article[4])); 
...
<div id="Container"> 
  ...
  <div id="ContentPane"> 
	...
	$replacement = array ("<P>\\n",
			 ...
			 "\\\'");
			 	$Articles = array();
				$Articles_query = tep_db_query("SELECT * FROM articles WHERE IssueID='" . $issueGet . "' ORDER BY StartPage ASC");
