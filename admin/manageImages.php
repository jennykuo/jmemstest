...
*/
 ...
 function assert_handler($file, $line, $code)
  ...
  assert($_SESSION["sessionUser"] == SITE_MANAGEMENT_USERNAME); 
	...
	if (isset($_POST['action']) && ($_POST['action'] == 'processDeleteImages')) {
		$files = split('@',$_POST["imagesToDelete"]);
	...
	if (isset($_POST['action']) && ($_POST['action'] == 'processEditImage')) {
		$file = mysql_real_escape_string($_POST["imageToEdit"]);
	...
	if(sizeof($_FILES)) {
		// uploaded a file
		if ($_FILES['imageFile']['name'] != '' && $_FILES["imageFile"]['size'] >= 1) {
			$uploadfile = '../' . DIR_WS_COVER_IMAGES . basename($_FILES['imageFile']['name']);
			move_uploaded_file($_FILES['imageFile']['tmp_name'], $uploadfile);
		}
		tep_db_query("INSERT INTO images (Filename) VALUES('" . basename($_FILES['imageFile']['name']) . "')");
	}	
